...
- **never** switch login, `.env` or checkout to get past a guard — the drafts-only
  desk and shared-login refusals have no flag on purpose, and a second login is the
  thing they exist to stop
- **never** suggest `--not-created-here`, `--other-operator` or `--older-than` as
  your own idea. Each is a sentence the operator says; you only relay it
