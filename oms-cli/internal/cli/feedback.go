...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "oms-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OMS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OMS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "oms-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/oms-pp-cli/feedback.jsonl.
When ` + "`OMS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OMS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  oms-pp-cli feedback list
  oms-pp-cli feedback list --limit 5
  oms-pp-cli feedback list --json`,
