...
func newOrdersFlowConfigCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "flow-config",
		Short:       "Global order approval-flow configuration",
		Example:     "  oms-pp-cli orders flow-config",
		Annotations: map[string]string{"pp:endpoint": "orders.flow-config", "pp:method": "GET", "pp:path": "/api/orders/flow-config/", "mcp:read-only": "true"},
