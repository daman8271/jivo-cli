...
type Config struct {
	...
	OmsToken      string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://oms.jivo.in",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("OMS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "oms-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("OMS_TOKEN"); v != "" {
		cfg.OmsToken = v
		cfg.markEnvOverride("OmsToken")
		cfg.AuthSource = "env:OMS_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.OmsToken != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.OmsToken = ""
	delete(c.envOverrides, "OmsToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["OmsToken"] {
			out.OmsToken = c.fileConfig.OmsToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "OmsToken":
		c.fileConfig.OmsToken = c.OmsToken
	}
}
