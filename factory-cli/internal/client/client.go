...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "jivo-factory-pp-cli", "http")
...
func (c *Client) cacheKey(path string, params map[string]string) string {
	...
	// Responses are company-scoped (Company-Code header); a cache hit from a
	// different company would silently serve the wrong company's data.
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export JIVO_FACTORY_TOKEN=<your-token> or jivo-factory-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.JivoFactoryToken)
	}
