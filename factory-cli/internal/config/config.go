...
type Config struct {
	...
	JivoFactoryToken string            `toml:"factory_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://factory.jivo.in/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("JIVO_FACTORY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "jivo-factory-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("JIVO_FACTORY_TOKEN"); v != "" {
		cfg.JivoFactoryToken = v
		cfg.markEnvOverride("JivoFactoryToken")
		cfg.AuthSource = "env:JIVO_FACTORY_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.JivoFactoryToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("JIVO_FACTORY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.JivoFactoryToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:JIVO_FACTORY_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"factory_token":      c.JivoFactoryToken,
			"JIVO_FACTORY_TOKEN": c.JivoFactoryToken,
			"token":              c.JivoFactoryToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.JivoFactoryToken = ""
	delete(c.envOverrides, "JivoFactoryToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["JivoFactoryToken"] {
			out.JivoFactoryToken = c.fileConfig.JivoFactoryToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "JivoFactoryToken":
		c.fileConfig.JivoFactoryToken = c.JivoFactoryToken
	}
}
