...
Run 'jivo-factory-pp-cli doctor' to verify auth and connectivity.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'jivo-factory-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")

	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		// Resolve the company scope once, up front: the --company flag wins,
		// then JIVO_FACTORY_COMPANY. Both are validated here so an invalid
		// value fails fast with exit 2 instead of silently defaulting. The
		// client reads the normalized env value on every request.
		companyRaw := companyFlag
		if companyRaw == "" {
			companyRaw = os.Getenv("JIVO_FACTORY_COMPANY")
		}
		flags.companyExplicit = strings.TrimSpace(companyRaw) != ""
		companyCode, companyErr := client.NormalizeCompany(companyRaw)
		if companyErr != nil {
			return usageErr(companyErr)
		}
		flags.companyCode = companyCode
		if err := os.Setenv("JIVO_FACTORY_COMPANY", companyCode); err != nil {
			return err
		}
	...
	rootCmd.AddCommand(newActivityCenterCmd(flags))
	...
	rootCmd.AddCommand(newOrderProcessingCmd(flags))
	...
	rootCmd.AddCommand(newSupplyChainCmd(flags))
