...
func newMaintenanceMaintenanceFireMovementsCmd(flags *rootFlags) *cobra.Command {
	var flagMovementType string
	var flagFireItem int
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flagFireItem != 0 {
				params["fire_item"] = formatCLIParamValue(flagFireItem)
			}
			if flagWorkOrder != 0 {
	...
	cmd.Flags().StringVar(&flagMovementType, "movement-type", "", "RECEIPT | ISSUE | CONSUME | RETURN | ADJUSTMENT — verified live (movement_type=ISSUE -> 4 of 5)")
	cmd.Flags().IntVar(&flagFireItem, "fire-item", 0, "verified live (fire_item=4 -> 3 rows) int")
