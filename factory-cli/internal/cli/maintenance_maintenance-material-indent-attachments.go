...
func newMaintenanceMaintenanceMaterialIndentAttachmentsCmd(flags *rootFlags) *cobra.Command {
	var flagIndent int
	...
	cmd.Flags().IntVar(&flagIndent, "indent", 0, "verified live (indent=1 -> 0 rows; the only attachment belongs to indent 17) int")
	cmd.Flags().StringVar(&flagDocType, "doc-type", "", "observed value: OTHER. Filter support INFERRED")
