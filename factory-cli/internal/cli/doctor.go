...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jivo-factory-pp-cli doctor
  jivo-factory-pp-cli doctor --json
  jivo-factory-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export JIVO_FACTORY_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("JIVO_FACTORY_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "JIVO_FACTORY_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JIVO_FACTORY_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'jivo-factory-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'jivo-factory-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'jivo-factory-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'jivo-factory-pp-cli sync' to refresh."
	}
	return report
}
