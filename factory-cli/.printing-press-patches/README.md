...
| [[CLI/factory-cli/.printing-press-patches/0005-preserve-oil-release-pagination|0005 — preserve Oil release pagination]] | Always send the pagination pair required by the upstream HANA view. | `internal/cli/barcode_production-release-oil.go` |
...
5. Confirm `barcode production-release-oil --help` includes `--page` and `--page-size`.
...
7. Run `gofmt`, `go test ./...`, `go vet ./...`, and `go build ./...`.
