...
| [[CLI/factory-cli/.printing-press-patches/0009-bind-mcp-path-placeholders|0009 — bind MCP path placeholders]] | Bind `{id}`-style path placeholders in both MCP dispatch paths. | `internal/mcp/code_orch.go`; `internal/mcp/code_orch_pathparam_test.go` |
| [[CLI/factory-cli/.printing-press-patches/0010-add-three-verified-get-endpoints|0010 — add three verified GET endpoints]] | Keep `maintenance scan-lookup`, `quality-control production-qc-run-sessions`, `gate-core raw-material-gate-entry-po-receipts`, and the corrected `raw-material-gate-entry` id description. **`/po-receipts/view` is on the never-publish list — read the Open conflict section before trusting a green invariant run.** | `spec.yaml`; the three `internal/cli/*.go` command files; `internal/mcp/code_orch.go`; `tools-manifest.json` |
