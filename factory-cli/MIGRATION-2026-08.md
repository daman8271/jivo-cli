...
| `security-checks …/security/view`, `weighment …/weighment/view` | Same `get_or_create` shape — a child record keyed by its parent's gate-entry id, and both return a **single object**. Never probed, because the only way to test is to call one for a parent with no child, which is the act that would create it. |
| ~~`raw-material-gatein …/po-receipts/view`~~ | **CLEARED 2026-08-31 and now shipped** as `gate-core raw-material-gate-entry-po-receipts`. It was grouped with the two above by name shape; the response shape separates it. On the same entry 3707 (`research/refute-factory-2026-08-22.md:108-110`) po-receipts returns an **array** `[{"id":1087,…}]` while security/view returns an **object** `{"id":1207,…}` — a `get_or_create` returns one object, a `many=True` list serializer cannot create. Also: 192 raw-material gate entries across all three companies each have ≥1 po_receipt, so the create branch has no reachable input in live data. Residual ~10%: a view could `get_or_create` then serialize `many=True` over a queryset. Daman's call. |
