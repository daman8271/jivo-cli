...
| `DESKTOP-73N6JE8` (Ziyaul's) | 23011 | `ziyaul` | ⛔ **UNREACHABLE since 2026-08-05**: tunnel up, sshd dead. 2026-08-21 17:06 ran **v7** on the box: registered OK, `REACHABLE : NO - tunnel-up-but-sshd-silent`, `SSHD : Stopped`, routes `windows-update: skipped, sshd was already present` -- sshd present but **refuses to start**, cause not named by v7. Also a Tailscale peer of the VPS (`desktop-73n6je8`, 100.96.50.42, tag:scraper) with 22 closed there too, so no remote path. **Next: run v8** (repair ladder) on the box and read its `repair:` lines |
