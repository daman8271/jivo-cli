...
| `DESKTOP-73N6JE8` (Ziyaul's) | 23011 | `ziyaul` | ⛔ **UNREACHABLE since 2026-08-05**: tunnel up, sshd dead. 2026-08-21 17:06 ran **v7** on the box: registered OK, `REACHABLE : NO - tunnel-up-but-sshd-silent`, `SSHD : Stopped`, routes `windows-update: skipped, sshd was already present` -- sshd present but **refuses to start**, cause not named by v7. No remote path exists (the tunnel is the dead thing). **2026-08-21 17:40 ran v8**: ladder stopped at rung 3 (logger bug, fixed in v9). **Next: run v9** (`ziyaul.cmd`) and read its `repair:` lines |
