...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "jivo-ecom-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JIVO_ECOM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JIVO_ECOM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "jivo-ecom-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/jivo-ecom-pp-cli/feedback.jsonl.
When ` + "`JIVO_ECOM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JIVO_ECOM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jivo-ecom-pp-cli feedback list
  jivo-ecom-pp-cli feedback list --limit 5
  jivo-ecom-pp-cli feedback list --json`,
