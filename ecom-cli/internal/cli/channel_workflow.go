...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  jivo-ecom-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  jivo-ecom-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"chatbot", "dashboard", "notifications", "shipment", "uploads"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  jivo-ecom-pp-cli workflow status

  # Show status as JSON
  jivo-ecom-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("jivo-ecom-pp-cli")
			}
