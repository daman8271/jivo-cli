...
func newDashboardRealiseWaterfallCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "realise-waterfall",
		Short:       "Realise dashboard: waterfall (net realisation analytics)",
		Example:     "  jivo-ecom-pp-cli dashboard realise-waterfall",
		Annotations: map[string]string{"pp:endpoint": "dashboard.realise-waterfall", "pp:method": "GET", "pp:path": "/api/dashboard/realise-waterfall", "mcp:read-only": "true"},
	...
	cmd.Flags().StringVar(&flagItemHead, "item-head", "", "item-head key; omitted when blank; waterfall has no call site in the downloaded chunks")
	cmd.Flags().StringVar(&flagCategory, "category", "", "category name; omitted when blank; waterfall has no call site in the downloaded chunks")
