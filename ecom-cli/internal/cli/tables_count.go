...
func newTablesCountCmd(flags *rootFlags) *cobra.Command {
	var flagTable string

	cmd := &cobra.Command{
		Use:         "count",
		Short:       "Row count for a single table",
		Example:     "  jivo-ecom-pp-cli tables count --table all_platform_inventory",
		Annotations: map[string]string{"pp:endpoint": "tables.count", "pp:method": "GET", "pp:path": "/api/dashboard/table-count/{table}", "mcp:read-only": "true"},
