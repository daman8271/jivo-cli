...
func newPlatformPrimaryMonthTargetsDashboardCmd(flags *rootFlags) *cobra.Command {
	var flagMonth string
	var flagYear string
	var flagNocache string

	cmd := &cobra.Command{
		Use:         "primary-month-targets-dashboard",
		Short:       "Primary month-targets dashboard across platforms Server requires: `month` (1-12) and `year` (YYYY) are required.",
		Example:     "  jivo-ecom-pp-cli platform primary-month-targets-dashboard --month example-value --year example-value",
		Annotations: map[string]string{"pp:endpoint": "platform.primary-month-targets-dashboard", "pp:method": "GET", "pp:path": "/api/platform/primary-month-targets/dashboard", "mcp:read-only": "true"},
