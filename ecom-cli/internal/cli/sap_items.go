...
func newSapItemsCmd(flags *rootFlags) *cobra.Command {
	var flagPage string
	var flagPageSize string

	cmd := &cobra.Command{
		Use:         "items",
		Short:       "SAP item master. Company scope: JIVO MART (JIVO_MART_HANADB), not Oil and not group-wide.",
		Example:     "  jivo-ecom-pp-cli sap items",
		Annotations: map[string]string{"pp:endpoint": "sap.items", "pp:method": "GET", "pp:path": "/api/sap/items", "mcp:read-only": "true"},
