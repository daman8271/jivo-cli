...
func newPlatformInventoryMatchCmd(flags *rootFlags) *cobra.Command {
	var flagPlatform string
	var flagSku string

	cmd := &cobra.Command{
		Use:         "inventory-match",
		Short:       "Inventory reconciliation/match for a platform",
		Example:     "  jivo-ecom-pp-cli platform inventory-match --platform amazon --sku example-value",
		Annotations: map[string]string{"pp:endpoint": "platform.inventory-match", "pp:method": "GET", "pp:path": "/api/platform/{platform}/inventory-match", "mcp:read-only": "true"},
	...
	cmd.Flags().StringVar(&flagSku, "sku", "", "SKU code; sku is hard-wired as the only query key by the service function itself")
