...
In local mode: searches locally synced data only.`,
  ...
  jivo-ecom-pp-cli search "error timeout"

  # Force local search only
  jivo-ecom-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  jivo-ecom-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  jivo-ecom-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'jivo-ecom-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/jivo-ecom-pp-cli/data.db)")
