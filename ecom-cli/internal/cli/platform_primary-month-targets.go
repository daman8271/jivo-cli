...
func newPlatformPrimaryMonthTargetsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "primary-month-targets",
		Short:       "Primary monthly targets for a platform",
		Example:     "  jivo-ecom-pp-cli platform primary-month-targets --platform amazon",
		Annotations: map[string]string{"pp:endpoint": "platform.primary-month-targets", "pp:method": "GET", "pp:path": "/api/platform/{platform}/primary-month-targets", "mcp:read-only": "true"},
	...
	cmd.Flags().IntVar(&flagMonth, "month", 0, "month number / 4-digit year")
	cmd.Flags().IntVar(&flagYear, "year", 0, "month number / 4-digit year")
