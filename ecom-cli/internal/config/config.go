...
type Config struct {
	...
	JivoEcomToken string            `toml:"ecom_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://ecom.jivo.in",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("JIVO_ECOM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "jivo-ecom-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("JIVO_ECOM_TOKEN"); v != "" {
		cfg.JivoEcomToken = v
		cfg.markEnvOverride("JivoEcomToken")
		cfg.AuthSource = "env:JIVO_ECOM_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.JivoEcomToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("JIVO_ECOM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.JivoEcomToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:JIVO_ECOM_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"ecom_token":      c.JivoEcomToken,
			"JIVO_ECOM_TOKEN": c.JivoEcomToken,
			"token":           c.JivoEcomToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.JivoEcomToken = ""
	delete(c.envOverrides, "JivoEcomToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["JivoEcomToken"] {
			out.JivoEcomToken = c.fileConfig.JivoEcomToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "JivoEcomToken":
		c.fileConfig.JivoEcomToken = c.JivoEcomToken
	}
}
