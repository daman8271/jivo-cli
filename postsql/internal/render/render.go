...
// project narrows a result to the --select columns. An empty selector means
// "all columns". A non-empty selector that names any column not present in the
// result is an error — we never silently fall back to the full column set,
// since a single typo would otherwise dump far more (or different) data than
