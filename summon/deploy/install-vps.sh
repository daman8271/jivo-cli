...
say "fleet ssh config"
# The VPS is the tunnel ENDPOINT, so each box sits on 127.0.0.1:<tunnel_port>
# directly — no ProxyJump, unlike the Mac. Generate a dedicated config from
# policy.json and leave root's own ~/.ssh/config strictly alone: fleet-health and
# fleet-auto-repair depend on it, and clobbering it would break more than this.
python3 - "$ROOT" <<'SSHPY'
import json, os, sys
root = sys.argv[1]
pol = json.load(open(os.path.join(root, "policy.json"), encoding="utf-8"))
out = [
    "# Generated by install-vps.sh from policy.json. Do not hand-edit —",
    "# add the box to policy.json instead and re-run the installer.",
    "#",
    "# The VPS is the reverse-tunnel endpoint, so every box is on loopback here.",
    "",
    "Host *",
    "    StrictHostKeyChecking accept-new",
    "    UserKnownHostsFile ~/.ssh/known_hosts.jivo-summon",
...
n = 0
for box, b in sorted(pol.get("boxes", {}).items()):
    port = b.get("tunnel_port")
    user = b.get("ssh_user", "")
    if box == "vps":
        continue
    if not port or not user:
        out.append(f"# {box}: skipped — policy has no {'tunnel_port' if not port else 'ssh_user'}")
        continue
    out += [
        f"Host {box}",
        ...
        f"    Port {port}",
        f"    User {user}",
        f"    HostKeyAlias jivo-summon-{box}",
        "",
    ]
    n += 1
path = os.path.join(root, "ssh_config")
tmp = path + ".tmp"
open(tmp, "w", encoding="utf-8").write("\n".join(out) + "\n")
os.chmod(tmp, 0o600)
os.replace(tmp, path)
print(f"wrote {path} with {n} box(es)")
SSHPY
