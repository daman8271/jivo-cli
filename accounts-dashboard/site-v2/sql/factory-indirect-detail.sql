-- Same window as factory-indirect.sql, one row per GL account per month, so the
-- page can pivot GL x month. ACCT_CODE stays text (it is an identifier).
WITH W AS (
