# Rosetta — `IncomingPayments` (API) ↔ `ORCT` (HANA) · OIL

Derived by value-matching 8 real document(s) (`DocEntry` 22045, 22043, 22042, 22039, 22038, 22036, 22035, 22033) read through both doors. A pair had to agree on **every** document to be reported.

- **14** properties mapped to exactly one column
- **7** of those have a **different name** on the two sides ← the dangerous ones
...
## Same name on both sides (7)

`BPLName`, `CardCode`, `CardName`, `DocEntry`, `PayToCode`, `Series`, `VATRegNum`
...
| `U_Pay_Report_Gen` | `ApplyVAT`, `Canceled`, `DPPStatus`, `DiffCurr`, `DigPayment`, `EnblDpmTax`, `Handwrtten`, `IsPaytoBnk`, `PaymType`, `PmntWTCert`, `Printed`, `Proforma`, `ShowAtCard`, `SpiltTrans`, `SpltCredLn`, `Status`, `Submitted`, `U_Pay_Report_Gen`, `WizDunBlck`, `confirmed` |
