...
# OMS_EXPECT_USER · JIVO_FACTORY_CLI · OMS_BIN · JIVO_ECOM_WRAPPER.
...
FACTORY_CLI="${JIVO_FACTORY_CLI:-$REPO_DIR/factory-cli/jivo-factory-pp-cli}"
OMS_CLI="${OMS_BIN:-$REPO_DIR/oms-cli/oms-pp-cli}"
OMS_CFG="${OMS_CONFIG:-$HOME/.config/oms-pp-cli/oms-daman.toml}"
OMS_EXPECT_USER="${OMS_EXPECT_USER:-Daman@oms.com}"
ECOM="${JIVO_ECOM_WRAPPER:-$HOME/.local/bin/ecom}"
[ -x "$ECOM" ] || ECOM="$(command -v ecom || echo "$ECOM")"
...
if wants ecom; then
  if [ ! -x "$ECOM" ]; then
    say "ecom    SKIP — wrapper not executable at $ECOM (set JIVO_ECOM_WRAPPER)"; SKIPPED=$((SKIPPED+1))
  else
