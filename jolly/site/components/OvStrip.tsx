...
function money(rs: number) {
  if (rs <= 0) return "—";
  return rs >= 1e7 ? `₹${(rs / 1e7).toFixed(2)} Cr` : `₹${(rs / 1e5).toFixed(1)} L`;
}
...
export default function OvStrip({ days }: { days: D[] }) {
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
      ...
      <div className="min-h-[62px] border-b border-zinc-800 pb-3 mb-4">
        {d === null ? (
          <div className="text-sm text-zinc-500 pt-3">
            Hover a day for its numbers. Click to open that day. Bar height is litres made; colour is how full the
            godown was.
          </div>
        ) : (
          <div className="flex flex-wrap items-start gap-x-6 gap-y-3">
            <div className="min-w-[110px]">
              <div className="text-[10px] uppercase tracking-wider text-zinc-500">
                {d.working ? "Working day" : "Sunday — off"}
              </div>
              <div className="text-sm font-semibold mt-0.5">{label(d.date)}</div>
            </div>
            <Stat k="Made" v={d.made_l > 0 ? `${inr(d.made_l)} L` : "nothing"} />
            <Stat k="Value" v={money(d.value)} />
            ...
            <Stat k="Blocked" v={inr(d.blocked)} tone={d.blocked > 0 ? "text-red-300" : "text-zinc-100"} />
            <Stat k="New order lines" v={inr(d.new_orders)} />
            <Stat k="Buy lines" v={inr(d.bought)} />
          </div>
        )}
      </div>
        ...
        {days.map((x, i) => {
          ...
          return (
            <Link
              ...
              aria-label={`${new Date(x.date + "T00:00:00Z").getUTCDate()} August — made ${inr(
                x.made_l
              )} litres, godown ${x.storage_pct} per cent full`}
      ...
      {/* storage ribbon — reads even on the days nothing was made */}
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-4 text-[11px] text-zinc-500">
        ...
        <span className="flex items-center gap-1.5">
          <i className="w-3 h-3 rounded-[2px] bg-red-500 inline-block" /> 95% and over — nothing more fits
        </span>
        <span className="flex items-center gap-1.5">
          <i className="w-3 h-3 rounded-[2px] bg-zinc-700 inline-block" /> nothing made
        </span>
        <span className="opacity-60">Sundays dimmed</span>
      </div>
    </div>
  );
}
