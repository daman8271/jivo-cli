...
export default function OverviewStrip({ days }: { days: StripDay[] }) {
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
      ...
      <div className="min-h-[62px] border-b border-zinc-800 pb-3 mb-4">
        {d === null ? (
          <div className="text-sm text-zinc-500 pt-3">
            Move over a day to see its numbers. Click to open that day. Bar height = litres made. Colour = how full
            the godown is. Nothing here has happened yet.
          </div>
        ) : (
          <div className="flex flex-wrap items-start gap-x-6 gap-y-3">
            <div className="min-w-[110px]">
              <div className="text-[10px] uppercase tracking-wider text-zinc-500">
                {d.working ? "Working day — plan" : `${d.weekday} — factory closed`}
              </div>
            ...
            <Stat k="Worth" v={money(d.value_rs)} />
            <Stat k="Billed" v={d.shipped_l > 0 ? `${inr(d.shipped_l)} L` : "nothing"} />
            <Stat
              k="Machines busy"
              v={`${d.util}%`}
            ...
            <Stat k="Stuck runs" v={inr(d.blocked)} tone={d.blocked > 0 ? "text-red-300" : "text-zinc-100"} />
            <Stat k="Confirmed orders" v={inr(d.real_rows)} />
            <Stat k="Expected — not ordered" v={inr(d.forecast_rows)} tone="text-sky-300" />
          </div>
        )}
      </div>
        ...
        {days.map((x, i) => {
          ...
          return (
            <Link
              ...
              aria-label={`${dlabel(x.date)} — the plan makes ${inr(x.made_l)} litres, godown ${
                x.storage_pct
              } per cent full`}
      ...
      {/* godown ribbon — reads even on the days nothing is made */}
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-4 text-[11px] text-zinc-500">
        <span className="text-zinc-400">How full the godown is:</span>
        ...
        <span className="flex items-center gap-1.5">
          <i className="w-3 h-3 rounded-[2px] bg-red-500 inline-block" /> 95% and over — nearly full: production is
          slowed to what can leave
        </span>
        <span className="flex items-center gap-1.5">
          <i className="w-3 h-3 rounded-[2px] bg-zinc-700 inline-block" /> nothing made
        </span>
        <span className="opacity-60">Sundays dimmed · every bar is the plan</span>
      </div>
    </div>
  );
}
