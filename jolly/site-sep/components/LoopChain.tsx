// The block -> order -> land -> run loop, rendered so it stays visible ACROSS days:
...
export function LoopDayRef({ date, map, className = "" }: { date: string; map: Record<string, number>; className?: string }) {
  const n = map[date];
  if (!n) {
    return <span className={`text-zinc-500 ${className}`}>{dlabel(date)} — beyond September</span>;
  }
  return (
...
/** Which chain explains a binder that is blocking product on `today`? */
...
/**
 * One chain as a four-step pipeline. `today` (ISO date) highlights the step this page's
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-3">
      <div className="mb-2 flex flex-wrap items-center gap-2">
        ...
        <Pill tone={chain.kind === "OIL" ? "red" : "zinc"}>{chain.kind === "OIL" ? "oil" : "packaging"}</Pill>
        {roles.map((r) => (
          <Pill key={r} tone="amber">{r}</Pill>
        ))}
        <span className="ml-auto text-[11px] text-violet-300/70" title="This loop is the simulator's plan — nothing has been ordered or run.">simulated loop</span>
      </div>
      <div className="flex flex-col gap-2 sm:flex-row">
        <Step title="the wall" done>
          held {chain.fg_codes_blocked.length} product{chain.fg_codes_blocked.length === 1 ? "" : "s"}
          <span className="ml-1 text-xs text-zinc-500" title={chain.fg_codes_blocked.join(", ")}>
            ({chain.fg_codes_blocked.slice(0, 3).join(", ")}
            {chain.fg_codes_blocked.length > 3 ? ", …" : ""})
          </span>
        </Step>
        <Arrow />
        <Step title="ordered" active={chain.ordered_day === today} done={chain.ordered_day < today}>
          ...
          <div className="text-xs text-zinc-500">lead {chain.lead_days} d</div>
        </Step>
        <Arrow />
        <Step
          title={resolved ? "lands — unblocked" : "lands"}
        ...
        >
          ...
          {typeof chain.waited_days === "number" ? <div className="text-xs text-zinc-500">waited {chain.waited_days} d</div> : null}
          {!resolved && chain.note ? <div className="text-xs text-amber-300/80">{chain.note}</div> : null}
        </Step>
        <Arrow />
        <Step title="back on the line" active={fr?.day === today} done={Boolean(fr && fr.day < today)}>
          {fr ? (
            <>
            ...
            <span className="text-xs text-zinc-500">no freed product ran before month end</span>
          ) : (
