# Jivo Printed CLI Agent Guide

This directory is a generated `jivo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
jivo-pp-cli doctor --json
jivo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
jivo-pp-cli which "<capability>" --json
jivo-pp-cli <command> --help
```
...
```bash
jivo-pp-cli <command> --help
jivo-pp-cli <command> --dry-run --agent
```
