...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "jivo-pp-cli",
		Short: "Manage jivo resources via the jivo API",
		Long: `Manage jivo resources via the jivo API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'jivo-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("jivo-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'jivo-pp-cli profile list')")
	...
	rootCmd.AddCommand(newMasterdataCmd(flags))
	rootCmd.AddCommand(newOihCmd(flags))
	...
	// import removed: generic JSONL-driven POST is the only write vector the
	// generator ships; this CLI is strictly READ-ONLY against a live production
	// ERP. (newImportCmd remains defined but unregistered; re-apply this removal
	// after any `generate --force`.)
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newSyncCmd(flags))
	rootCmd.AddCommand(newWorkflowCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newCreditPromotedCmd(flags))
	rootCmd.AddCommand(newAuthCmd(flags)) // hand-authored: Django session login (jivo_login.go)
