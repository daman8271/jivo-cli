...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "jivo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JIVO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JIVO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "jivo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/jivo-pp-cli/feedback.jsonl.
When ` + "`JIVO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JIVO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jivo-pp-cli feedback list
  jivo-pp-cli feedback list --limit 5
  jivo-pp-cli feedback list --json`,
