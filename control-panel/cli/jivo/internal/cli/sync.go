...
Resource scoping:
  ...
  jivo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  jivo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  jivo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  jivo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  jivo-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/jivo-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"inventory":  "/inventory/production/api/fg-list/",
		"masterdata": "/realise/api/rate-list/",
		"targets":    "/realise/api/targets/",
		"users":      "/users/",
	}
...
func syncHTMLExtractionOptions(resource string, parentResource string, baseURL string, contentType string, path string, params map[string]string) (htmlExtractionOptions, bool) {
	...
	case "users":
		return htmlExtractionOptions{
			...
			LinkPrefixes:   []string{},
			Limit:          0,
			ScriptSelector: "script#um-users",
			JSONPath:       "",
		}, true
	default:
