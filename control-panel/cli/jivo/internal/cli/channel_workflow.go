...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  jivo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  jivo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"inventory", "masterdata", "targets", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  jivo-pp-cli workflow status

  # Show status as JSON
  jivo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("jivo-pp-cli")
			}
