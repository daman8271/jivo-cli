...
This directory records the four load-bearing deviations from the generated
...
| [[CLI/exim/cli/exim-pp-cli/.printing-press-patches/mcp-get-only-guard|MCP GET-only guard]] | Fail closed on non-GET in both MCP execution paths; register zero write tools. |

## Reprint checklist

1. Re-apply all four patches.
...
6. Confirm `internal/mcp/readonly_guard_test.go` still passes — it pins the
   advertised MCP tool set and both fail-closed method guards.
