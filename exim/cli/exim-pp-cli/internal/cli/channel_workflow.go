...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  exim-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  exim-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "daily-price", "daily-price-db-list", "daily-price-highest-lowest", "daily-price-trends", "dc", "director-inventorty", "exim-rates", "items", "items-rm", "items-rm-summary", "items-rm-varieties", "jivo-rate", "license", "license-advance-license-export-lines", "license-advance-license-import-lines", "parties", "rates", "rates-basic-rate", "rates-commodity", "rates-market-rate-get", "rates-market-rate-latest", "rates-rate-table-latest", "sap-sync", "sap-sync-balance-sheet", "sap-sync-custa-balance-sheet", "sap-sync-customer-aging-balance", "sap-sync-finished-inventory", "sap-sync-inventory", "sap-sync-open-ar", "sap-sync-open-pos", "sap-sync-planned-months", "sap-sync-vendor-balance-sheet", "stock-status", "stock-status-contractual-history", "stock-status-debit-entries", "stock-status-debit-insights", "stock-status-stock-dashboard", "stock-status-stock-insights", "stock-status-stock-summary", "sync-logs", "tank", "tank-capacity-insights", "tank-in-tank-items", "tank-item-wise-summary", "tank-items", "tank-log", "tank-tank-summary"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  exim-pp-cli workflow status

  # Show status as JSON
  exim-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("exim-pp-cli")
			}
