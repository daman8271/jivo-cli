...
// TestRootCommandOmitsGenericImport pins the EXIM-specific command-tree
// customization: the generated generic import implementation may remain in
// the source tree for clean reprints, but it must never be registered or
// runnable because it issues arbitrary POST create/upsert requests.
func TestRootCommandOmitsGenericImport(t *testing.T) {
	rootCmd := newRootCmd(&rootFlags{})
	for _, command := range rootCmd.Commands() {
		if command.Name() == "import" {
			t.Fatal("root command unexpectedly registers generic import")
		}
	}
	...
	rootCmd.SetArgs([]string{"import", "stock-status", "--input", "records.jsonl"})
	err := rootCmd.Execute()
	if err == nil {
		t.Fatal("generic import invocation unexpectedly succeeded")
	}
	if want := `unknown command "import"`; !strings.HasPrefix(err.Error(), want) {
		t.Fatalf("import invocation error = %q, want prefix %q", err, want)
	}
}
