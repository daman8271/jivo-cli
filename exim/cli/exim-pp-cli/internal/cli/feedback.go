...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "exim-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXIM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXIM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "exim-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/exim-pp-cli/feedback.jsonl.
When ` + "`EXIM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXIM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exim-pp-cli feedback list
  exim-pp-cli feedback list --limit 5
  exim-pp-cli feedback list --json`,
