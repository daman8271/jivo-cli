...
type Config struct {
	...
	EximToken     string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://eximbe.jivo.in",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("EXIM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "exim-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EXIM_TOKEN"); v != "" {
		cfg.EximToken = v
		cfg.markEnvOverride("EximToken")
		cfg.AuthSource = "env:EXIM_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.EximToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("EXIM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.EximToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:EXIM_TOKEN"
		}
		return "Bearer " + c.EximToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.EximToken = ""
	delete(c.envOverrides, "EximToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["EximToken"] {
			out.EximToken = c.fileConfig.EximToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "EximToken":
		c.fileConfig.EximToken = c.EximToken
	}
}
